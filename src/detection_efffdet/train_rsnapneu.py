...
from effdet import create_model, unwrap_bench, create_loader, create_rsnapneu_dataset, create_evaluator
...
def main():
    ...
    output_dir = ''
    if args.local_rank == 0:
        output_base = args.output if args.output else './checkpoints/rsnapneu_pretrain'
        exp_name = '_'.join([
            args.model, str(args.image_size)
        ])
...
def create_datasets_and_loaders(
    ...
    dataset_train, dataset_eval = create_rsnapneu_dataset(name=args.dataset, root=args.root)
