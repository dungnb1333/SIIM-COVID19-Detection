...
def create_model(
        model_name, bench_task='', image_size=None, num_classes=None, pretrained=False,
        checkpoint_path='', checkpoint_ema=False, **kwargs):

    config = get_efficientdet_config(model_name)
    return create_model_from_config(
        config, bench_task=bench_task, image_size=image_size, num_classes=num_classes, pretrained=pretrained,
        checkpoint_path=checkpoint_path, checkpoint_ema=checkpoint_ema, **kwargs)


def create_model_from_config(
        config, bench_task='', image_size=None, num_classes=None, pretrained=False,
        checkpoint_path='', checkpoint_ema=False, **kwargs):
    ...
    if num_classes is not None and num_classes != config.num_classes:
        model.reset_head(num_classes=num_classes, image_size=image_size)
