...
from .parsers import *
from .dataset import DetectionDatset, XrayDetectionDatset
...
def create_rsnapneu_dataset(name, root, splits=('train', 'val')):
    if isinstance(splits, str):
        splits = (splits,)
    name = name.lower()
    root = Path(root)
...
def create_warmup_dataset(name, root, splits=('train', 'val')):
    if isinstance(splits, str):
        splits = (splits,)
    name = name.lower()
    root = Path(root)
