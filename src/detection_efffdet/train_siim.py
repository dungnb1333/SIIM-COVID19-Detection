...
from effdet import create_model, unwrap_bench, create_loader, create_xray_dataset, create_evaluator
...
def main():
    ...
    output_dir = ''
    if args.local_rank == 0:
        output_base = args.output if args.output else './checkpoints'
        exp_name = '_'.join([
            args.model, str(args.image_size), 'fold{}'.format(args.fold)
        ])
...
def create_datasets_and_loaders(
    ...
    dataset_train, dataset_eval = create_xray_dataset(name=args.dataset, root=args.root, fold=args.fold)
