...
from dataset import SiimCovidExtDataset
...
if __name__ == "__main__":
    ...
    for source in ['padchest', 'pneumothorax', 'vin']:
        ...
        dataset = SiimCovidExtDataset(df=ext_df, image_size=args.image_size)
                ...
                with amp_autocast():
                    for fold in args.folds:
                        ...
                        for idx, det_pred, flip_det_pred, img_size in zip(image_idxs, dets, flip_dets, image_sizes):
                            image_path = ext_df.loc[idx, 'image_path']
                            if image_path not in list(predict_dict.keys()):
                                predict_dict[image_path] = [[],[],[], img_size[0], img_size[1]]
                            ...
                            predict_dict[image_path][0] += [box_pred,flip_box_pred]
                            predict_dict[image_path][1] += [score_pred, flip_score_pred]
                            predict_dict[image_path][2] += [label_pred, flip_label_pred]
        
        pred_dict_path = 'predictions/{}_{}_fold{}_{}_pred.pth'.format(args.model, args.image_size, '_'.join(str(x) for x in args.folds), source)
