...
if __name__ == "__main__":
    ...
    test_dataset = SiimCovidCLSTestDataset(df=test_df, images_dir='../../dataset/siim-covid19-detection/images/test', image_size=cfg['aux_image_size'])
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=cfg['workers'], pin_memory=False, drop_last=False, collate_fn=collate_fn)
    ...
    pred_dict_path = 'predictions/{}_{}_fold{}_test_pred.pth'.format(cfg['backbone_name'], cfg['aux_image_size'], '_'.join(str(x) for x in args.folds))
