...
from dataset import RSNAPneuAuxDataset, rsnapneumonia_classes
...
if __name__ == "__main__":
    ...
    train_df = train_df.loc[train_df['hasbox'] == True].reset_index(drop=True)
    valid_df = valid_df.loc[valid_df['hasbox'] == True].reset_index(drop=True)
    ...
    model = SiimCovidAuxModel(
        ...
        num_classes=len(rsnapneumonia_classes),
    ...
    LOG = '{}/{}_{}_rsnapneu.log'.format(ckpt_dir, cfg['backbone_name'], cfg['aux_image_size'])
    CHECKPOINT = '{}/{}_{}_rsnapneu.pth'.format(ckpt_dir, cfg['backbone_name'], cfg['aux_image_size'])
