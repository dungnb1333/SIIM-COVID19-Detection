...
from utils import seed_everything, refine_det, collate_fn, save_dict
...
if __name__ == "__main__":
    ...
    output_dir = '{}/{}_{}_fold{}_test_pred'.format(args.output_dir, cfg['backbone_name'], cfg['aux_image_size'], '_'.join(str(x) for x in args.folds))
    ...
    for fold in args.folds:
        ...
        CHECKPOINT = '{}/{}_{}_fold{}.pth'.format(args.ckpt_dir, cfg['backbone_name'], cfg['aux_image_size'], fold)
    ...
    test_dataset = SiimCovidCLSTestDataset(df=test_df, images_dir=args.image_dir, image_size=cfg['aux_image_size'])
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=False, drop_last=False, collate_fn=collate_fn)

    predict_dict = {}
    for ids, images, targets, heights, widths in tqdm(test_loader):
