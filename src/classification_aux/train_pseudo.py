...
from dataset import SiimCovidAuxDataset, SiimCovidAuxPseudoDataset, classes
...
parser.add_argument("--weighted", default=False, type=lambda x: (str(x).lower() == "true"))
parser.add_argument("--ckpt_in_dir", default='checkpoints_v3', type=str)
parser.add_argument("--ckpt_out_dir", default='checkpoints_v4', type=str)
...
if __name__ == "__main__":
    ...
    cfg['workers'] = min(cfg['workers'], 6)
    print(cfg)

    os.makedirs(args.ckpt_out_dir, exist_ok = True)
    df = pd.read_csv('../../dataset/siim-covid19-detection/train_kfold.csv')
    pseudo_pub_test_df = pd.read_csv('pseudo_csv/pseudo_public_test.csv')
    ...
    for fold in args.folds:
        ...
        train_df['pseudo'] = np.array([False]*len(train_df), dtype=bool)
        train_df = pd.concat([train_df, pseudo_pub_test_df], ignore_index=True)

        train_dataset = SiimCovidAuxPseudoDataset(
            df=train_df,
        ...
        model = SiimCovidAuxModel(
            ...
            model_pretrained_path='{}/{}_{}_{}_aux_fold{}.pth'.format(args.ckpt_in_dir, cfg['encoder_name'], cfg['aux_image_size'], cfg['decoder'], fold), 
        ...
        LOG = '{}/{}_{}_{}_aux_fold{}.log'.format(args.ckpt_out_dir, cfg['encoder_name'], cfg['aux_image_size'], cfg['decoder'], fold)
        CHECKPOINT = '{}/{}_{}_{}_aux_fold{}.pth'.format(args.ckpt_out_dir, cfg['encoder_name'], cfg['aux_image_size'], cfg['decoder'], fold)
